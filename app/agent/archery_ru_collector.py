...
# Основной паттерн: дата ИДЁТ ПРЯМО ПЕРЕД ссылкой на новость (не обязательно
# в одном теге — между ними может быть закрывающий тег вроде </span>).
# Это отсекает ссылки в меню/навигации, у которых рядом даты нет.
_ITEM_RE = re.compile(
    r'(\d{2}\.\d{2}\.\d{4})\s*(?:</[a-zA-Z0-9]+>)?\s*'
    r'<a[^>]+href="(/page/[^"]+\.html)"[^>]*>\s*([^<]{5,200}?)\s*</a>',
    re.DOTALL,
)

# Запасной вариант — сильно ограничен: минимум 20 символов текста ссылки
# (реальные заголовки новостей длинные), чтобы не цеплять короткие пункты меню
# вроде "Тренерам" или "Судьям".
_FALLBACK_RE = re.compile(
    r'<a[^>]+href="(/page/[^"]+\.html)"[^>]*>([^<]{20,200})</a>'
)
